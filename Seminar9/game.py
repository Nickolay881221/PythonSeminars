...
def playground(dataPlay): 
    for item in range(0,len(dataPlay),3):
        context.bot.send_message(chat_id = chat_id, text="_"*13)
        update.message.reply_text(f"| {dataPlay[item]} | {dataPlay[item+1]} | {dataPlay[item+2]} |")
    update.message.reply_text("_"*13)
...
async def play_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(f'{update.effective_user.first_name} крестики-нолики, ваш ход>')
    await update.message.reply_text(f'___________\n'
                                    f'|{playData[0]}|{playData[1]}|{playData[2]}|\n'
                                    f'|{playData[3]}|{playData[4]}|{playData[5]}|\n'
                                    f'|{playData[6]}|{playData[7]}|{playData[8]}|\n'
                                    f'___________')
    """ await update.message.reply_text(playground(playData)) """
    """ await update.message.reply_text(playground(playData)) """
